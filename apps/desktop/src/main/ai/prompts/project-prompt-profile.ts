...
export const PROJECT_PROMPT_PROFILE_VERSION = 5;
...
Use the Write tool to create both files in the spec directory:
- \`spec.md\`
...
5. Update the subtask status in \`implementation_plan.json\` to \`completed\`. Use \`blocked\` or \`failed\` only when you cannot proceed.
