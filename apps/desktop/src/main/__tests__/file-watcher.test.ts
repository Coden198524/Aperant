...
describe('FileWatcher concurrency', () => {
  ...
  describe('deduplication: second watch() with same specDir is a no-op', () => {
    it('should only create one FSWatcher when watch() is called twice with the same specDir while the first is still in-flight', async () => {
      const specDir = '/project/.autocode/specs/001-task';
      const taskId = 'task-1';
  ...
  describe('supersession: watch() with different specDir replaces the in-flight call', () => {
    it('should let the second call win when the first is awaiting close()', async () => {
      const taskId = 'task-2';
      const specDir1 = path.join('/project', '.autocode', 'specs', '001-first');
      const specDir2 = path.join('/project', '.autocode', 'specs', '002-second');
    ...
    it('first watch() bails when pendingWatches changes to a different specDir', async () => {
      ...
      const specDir1 = path.join('/project', '.autocode', 'specs', 'super-first');
      const specDir2 = path.join('/project', '.autocode', 'specs', 'super-second');
  ...
  describe('project isolation: same taskId across projects does not collide', () => {
    it('keeps watchers isolated by projectId and only unwatches the targeted project', async () => {
      ...
      const specDirA = path.join('/project-a', '.autocode', 'specs', taskId);
      const specDirB = path.join('/project-b', '.autocode', 'specs', taskId);
  ...
  describe('cancellation: unwatch() during in-flight watch() prevents watcher creation', () => {
    it('should not create a watcher when unwatch() is called before the async gap resolves', async () => {
      const taskId = 'task-3';
      const specDir = '/project/.autocode/specs/003-cancel';
      ...
      const specDir2 = '/project/.autocode/specs/003-cancel-v2';
  ...
  describe('unwatchAll() cancels all pending watches', () => {
    it('should cancel pending watch() calls and clear pendingWatches', async () => {
      ...
      const specDir1 = '/project/.autocode/specs/004a';
      const specDir2 = '/project/.autocode/specs/004b';
      ...
      const newSpecDir1 = '/project/.autocode/specs/004a-v2';
      ...
      const specDirFresh = path.join('/project', '.autocode', 'specs', '004a-fresh');
  ...
  describe('getWatchedSpecDir()', () => {
    it('returns the specDir that was passed to watch()', async () => {
      const taskId = 'task-5';
      const specDir = path.join('/project', '.autocode', 'specs', '005-specdir');
    ...
    it('returns updated specDir after re-watch with different specDir', async () => {
      ...
      const specDir1 = path.join('/project', '.autocode', 'specs', '005b-first');
      const specDir2 = path.join('/project', '.autocode', 'specs', '005b-second');
