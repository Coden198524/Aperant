    ...
    >
      <CollapsibleTrigger className="flex w-full items-center justify-between text-xs font-medium hover:text-foreground">
        <div className="flex items-center gap-2">
          ...
          {t('context.projectIndex.labels.databaseModels', { defaultValue: 'Database Models' })} ({database.total_models})
        </div>
          ...
          return (
            <div key={modelName} className="flex items-start gap-2 text-xs">
              ...
              <span className="text-muted-foreground shrink-0 text-xs">
                {t('context.projectIndex.labels.fields', {
                  count: Object.keys(model.fields).length,
                  defaultValue: `${Object.keys(model.fields).length} fields`
                })}
              </span>
            </div>
          );
