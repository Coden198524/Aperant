    ...
    <div className="flex h-full items-center justify-center">
      <Card className="w-full max-w-lg p-8 text-center">
        ...
        <h2 className="text-xl font-semibold mb-2">
          {t('ideation.empty.title', { defaultValue: 'No Ideas Yet' })}
        </h2>
        <p className="text-muted-foreground mb-6">
          {t('ideation.empty.description', {
            defaultValue: "Generate AI-powered feature ideas based on your project's context, existing patterns, and target audience."
          })}
        </p>
        ...
        <div className="mb-6 p-4 bg-muted/50 rounded-lg text-left">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium">
              {t('ideation.empty.enabledTypes', { defaultValue: 'Enabled Ideation Types' })}
            </span>
            <Button
              variant="ghost"
              size="sm"
            ...
            {ALL_IDEATION_TYPES.map((type) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <TypeIcon type={type} />
                  <span className="text-sm">{getIdeationTypeLabel(t, type)}</span>
                </div>
                <Switch
        ...
        {hasToken === false && !isCheckingToken && (
          <p className="mt-3 text-sm text-muted-foreground">
            ...
            {t('errors.noProviderConfigured', {
              defaultValue: 'No AI provider configured. Add a provider account in Settings to generate ideas.'
            })}
          </p>
        )}
      </Card>
