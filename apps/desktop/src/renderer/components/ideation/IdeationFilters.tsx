  ...
  return (
    <Tabs value={activeTab} onValueChange={onTabChange} className="h-full flex flex-col">
      <TabsList className="shrink-0 mx-4 mt-4 flex-wrap h-auto gap-1">
        <TabsTrigger value="all">{t('memory.filters.all')}</TabsTrigger>
        <TabsTrigger value="code_improvements">
          <Zap className="h-3 w-3 mr-1" />
          {t('ideation.filters.code', { defaultValue: 'Code' })}
        </TabsTrigger>
        <TabsTrigger value="ui_ux_improvements">
          <Palette className="h-3 w-3 mr-1" />
          {t('ideation.filters.uiux', { defaultValue: 'UI/UX' })}
        </TabsTrigger>
        <TabsTrigger value="documentation_gaps">
          <BookOpen className="h-3 w-3 mr-1" />
          {t('ideation.filters.docs', { defaultValue: 'Docs' })}
        </TabsTrigger>
        <TabsTrigger value="security_hardening">
          <Shield className="h-3 w-3 mr-1" />
          {t('ideation.filters.security', { defaultValue: 'Security' })}
        </TabsTrigger>
        <TabsTrigger value="performance_optimizations">
          <Gauge className="h-3 w-3 mr-1" />
          {t('ideation.filters.performance', { defaultValue: 'Performance' })}
        </TabsTrigger>
      </TabsList>
      {children}
    </Tabs>
