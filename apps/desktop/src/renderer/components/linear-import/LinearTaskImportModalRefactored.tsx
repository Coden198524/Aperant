      ...
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] flex flex-col">
        <DialogHeader className="shrink-0">
          <DialogTitle className="flex items-center gap-2 text-foreground">
            ...
            {t('linearImport.title', { defaultValue: 'Import Linear Tasks' })}
          </DialogTitle>
          <DialogDescription>
            {t('linearImport.description', {
              defaultValue: 'Select tasks from Linear to import into AutoBuild'
            })}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="shrink-0">
          <Button variant="outline" onClick={() => handleOpenChange(false)}>
            {importResult?.success ? t('selection.done') : t('buttons.cancel')}
          </Button>
          {!importResult?.success && (
            <Button
            ...
            >
              {isImporting ? (
                <>
                  ...
                  {t('linearImport.importing', { defaultValue: 'Importing...' })}
                </>
              ) : (
                <>
                  <Download className="mr-2 h-4 w-4" />
                  {t('linearImport.importButton', {
