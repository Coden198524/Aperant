  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setArchiveError(result.error || t('archive.failed'));
      }
    } catch (err) {
      setArchiveError(err instanceof Error ? err.message : t('archive.failed'));
    } finally {
            ...
            >
              {isArchiving ? (
                ...
                <>
                  ...
                  {t('archive.action', { count: selectedTasks.length })}
                </>
              )}
            </Button>
          </div>
