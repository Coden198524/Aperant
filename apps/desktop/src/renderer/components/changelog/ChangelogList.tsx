...
export function ChangelogList({
  ...
  const getSummaryLabelKey = (): 'task' | 'commit' | 'item' => {
    switch (sourceMode) {
      case 'tasks':
        return 'task';
      case 'git-history':
                ...
                <div className="text-center py-12">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('list.noGitHistoryDescription')
                      : t('list.noBranchDiffDescription')}
                  </p>
                </div>
              </div>
            ) : (
        ...
        <Button onClick={onContinue} disabled={!canContinue} size="lg">
          ...
          {canContinue && (
            <Badge variant="secondary" className="ml-2">
              {t(`list.summary.${getSummaryLabelKey()}`, { count: getSummaryCount() })}
            </Badge>
          )}
        </Button>
      </div>
