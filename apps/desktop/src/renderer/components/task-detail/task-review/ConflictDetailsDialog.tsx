      ...
      <AlertDialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            ...
            {t('taskReview:conflictDetails.title', 'Merge Conflicts Preview')}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {t('taskReview:conflictDetails.summary', {
              count: mergePreview?.conflicts.length || 0,
              defaultValue: '{{count}} potential conflict detected.',
              defaultValue_plural: '{{count}} potential conflicts detected.'
            })}
            {mergePreview && mergePreview.summary.autoMergeable > 0 && (
              <span className="text-success ml-1">
                {t('taskReview:conflictDetails.autoMergeable', {
                  count: mergePreview.summary.autoMergeable,
                  defaultValue: '{{count}} can be auto-merged.'
                })}
              </span>
            )}
          </AlertDialogDescription>
        ...
        <div className="flex-1 overflow-auto min-h-0 -mx-6 px-6">
          {mergePreview?.conflicts && mergePreview.conflicts.length > 0 ? (
              ...
              {mergePreview.conflicts.map((conflict, idx) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {conflict.canAutoMerge && (
                        <Badge variant="secondary" className="text-xs bg-success/10 text-success">
                          {t('taskReview:conflictDetails.autoMergeBadge', 'auto-merge')}
                        </Badge>
                      )}
                    </div>
                  </div>
                  <div className="text-xs text-muted-foreground space-y-1">
                    {conflict.location && (
                      <div><span className="text-foreground/70">{t('taskReview:conflictDetails.location', 'Location:')}</span> {conflict.location}</div>
                    )}
                    {conflict.reason && (
                      <div><span className="text-foreground/70">{t('taskReview:conflictDetails.reason', 'Reason:')}</span> {conflict.reason}</div>
                    )}
                    {conflict.strategy && (
                      <div><span className="text-foreground/70">{t('taskReview:conflictDetails.strategy', 'Strategy:')}</span> {conflict.strategy}</div>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-8 text-muted-foreground">
              {t('taskReview:conflictDetails.empty', 'No conflicts detected')}
            </div>
          )}
        </div>
