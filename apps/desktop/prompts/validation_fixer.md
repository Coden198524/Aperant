...
- `requirement_model.md`: behavioral analysis and `RM-*` scenarios only.
...
- `design_model.md`: `SYS-*`, `DES-*`, `FLOW-*`, and applicable `CONTRACT-*`, `PAT-*`, and `REV-*` definitions only.
- `implementation_model.md`: exact repository bridge and `IMP-*` mappings only.
