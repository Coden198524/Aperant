export interface AutocodeDeepSeekNativeCliSession {
  id: string;
  cli: 'deepseek';
  title: string;
  updatedAt: string;
...
export interface AutocodeDeepSeekStoredSession {
  id: string;
  title?: string;
  projectPath?: string;
  isCLIMode?: boolean;
...
function getDeepSeekHistoryTitle(session: AutocodeDeepSeekStoredSession): string {
...
export function isDeepSeekStoredSession(session: AutocodeDeepSeekStoredSession): boolean {
...
export function deepSeekSessionsToNativeHistory(
  sessions: AutocodeDeepSeekStoredSession[],
): AutocodeDeepSeekNativeCliSession[] {
  const byId = new Map<string, AutocodeDeepSeekNativeCliSession>();
