...
export function buildAutocodePlanningStructuredOutputRetryPrompt(errorMessage: string): string {
  return [
    'RETRY TASKS WRITE',
    '',
    `Previous Write call failed before execution: ${compactAutocodeRetryLine(errorMessage, 260)}`,
    '',
    ...
    'Every executable task must include one _Depends on: ..._ line and one traceable _Evidence: ..._ line.',
    ...
    'A task covering more than three behaviors, more than three requirement/acceptance references, or more than four write-intent files is too broad.',
    'If split tasks touch the same file, keep them separate and add _Depends on: ..._ only for real prerequisites; the runtime queues overlapping file writes safely.',
...
export function buildAutocodePlanningStructuredOutputValidationRetryPrompt(errors: string[]): string {
  return [
    ...
    'Every executable task must include exactly one _Depends on: ..._ line; use none when there is no true prerequisite.',
...
export function buildAutocodeStandardTasksValidationRetryPrompt(errors: string[]): string {
  return [
    ...
    'Declare Tasks-Contract: 1 and keep every checkbox [ ]; runtime progress belongs only to implementation_plan.md.',
    'Preserve every completed historical definition unchanged. Restore a changed/removed completed ID and put revised work under a new task ID.',
    'Use checklist Markdown with phase items such as "- [ ] 1. Phase" and task items such as "- [ ] 1.1 Task".',
    'Every executable task must include _Depends on_, _Requirements_, _Design_, _Evidence_, _Done when_, and _Verification_. Include exact file intent.',
    'Reference R*/AC*/SCN*/E* and approved design IDs without copying their source prose.',
    'Evidence must cite E* IDs, context.md, project source/docs, existing project patterns, or verified official/industry references. Do not use "none" or vague guesses.',
    'Cover every requirement, scenario, acceptance criterion, or success criterion from spec.md/requirements.md; call out blocked or out-of-scope items instead of dropping them.',
    'Keep each executable task small enough for one focused coding session and include a clear done signal in guidance or _Done when: ..._.',
    'Split broad work into focused leaf tasks even when independent behavior touches the same file.',
    'Use _Depends on: none_ when there is no true prerequisite. Add real dependencies only for tasks that consume prior outputs or require ordered contracts.',
