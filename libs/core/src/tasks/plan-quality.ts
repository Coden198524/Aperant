...
export function buildAutocodePlanQualityRetryPrompt(errors: string[]): string {
  return [
    ...
    '- Preserve unaffected IDs, decisions, completed tasks, and change history.',
    ...
    `- Treat the Design-Contract: 5 package as binding. ${AUTOCODE_TASK_ARTIFACTS.design} owns architecture/ADR decisions and references ${AUTOCODE_TASK_ARTIFACTS.requirementModel}, ${AUTOCODE_TASK_ARTIFACTS.domainModel}, ${AUTOCODE_TASK_ARTIFACTS.designModel}, and ${AUTOCODE_TASK_ARTIFACTS.implementationModel}; keep ${AUTOCODE_TASK_ARTIFACTS.designReview} independent. Reject every other design contract version.`,
    '- Repair only the earliest affected owner and its downstream models. Preserve shared Design-Revision and unaffected stable IDs. RM/FUN/SSD belong in requirement_model.md, DOM in domain_model.md, SYS/DES/STATE/FLOW/CONTRACT/PAT/REV in design_model.md, and LANG/IMP in implementation_model.md.',
    '- Select the smallest complete project-consistent architecture. Declare forward-design, reverse-engineering, or mixed analysis; preserve requirement/observed/inferred/unresolved evidence provenance; allocate every RM-* and FUN-* through SYS-* before DES-*; and use REV-* only for evidence-backed source reconstruction.',
