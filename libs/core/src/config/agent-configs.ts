...
export const ALL_BUILTIN_TOOLS = [...BASE_READ_TOOLS, ...BASE_WRITE_TOOLS, ...WEB_TOOLS] as const;

/** Spec pipeline tools — read codebase + write to spec dir + web research. No Edit, no Bash. */
export const SPEC_TOOLS = [...BASE_READ_TOOLS, 'Write', ...WEB_TOOLS] as const;
...
export interface McpConfigResolveOptions {
...
function stringSettingEnabled(value: unknown, defaultValue: boolean): boolean {
  if (value === undefined) return defaultValue;
  return String(value).toLowerCase() !== 'false';
}

function stringSettingStrictTrue(value: unknown): boolean {
  return String(value ?? 'false').toLowerCase() === 'true';
}

/**
 * Resolve required MCP servers from the desktop-style MCP settings object.
 */
export function getRequiredMcpServersFromConfig(
  agentType: AgentType,
  ...
  const customServerIds =
    mcpConfig.CUSTOM_MCP_SERVERS?.map((server) => server.id).filter(Boolean) ?? [];

  const addValue = mcpConfig[`AGENT_MCP_${agentType}_ADD`];
  const removeValue = mcpConfig[`AGENT_MCP_${agentType}_REMOVE`];

  return getRequiredMcpServers(agentType, {
    projectCapabilities,
    linearEnabled:
      linearEnabled && stringSettingEnabled(mcpConfig.LINEAR_MCP_ENABLED, true),
    yunxiaoEnabled:
      yunxiaoEnabled && stringSettingEnabled(mcpConfig.YUNXIAO_MCP_ENABLED, true),
    memoryEnabled,
    electronMcpEnabled: stringSettingStrictTrue(mcpConfig.ELECTRON_MCP_ENABLED),
    puppeteerMcpEnabled: stringSettingStrictTrue(mcpConfig.PUPPETEER_MCP_ENABLED),
    context7Enabled: stringSettingEnabled(mcpConfig.CONTEXT7_ENABLED, true),
    agentMcpAdd: typeof addValue === 'string' ? addValue : undefined,
    agentMcpRemove: typeof removeValue === 'string' ? removeValue : undefined,
    customServerIds,
  });
}
