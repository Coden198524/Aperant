...
import type { BuiltinProvider } from '../auth/types.js';
import type { ReasoningConfig, ReasoningType } from './types.js';
...
export type ModelCatalogProvider = Exclude<BuiltinProvider, 'bedrock'>;
...
export const ALL_AVAILABLE_MODELS: ModelOption[] = [
  ...
  // OpenAI
  { value: 'gpt-5.6-sol', label: 'GPT-5.6 SOL', provider: 'openai', description: 'Latest flagship', capabilities: { thinking: true, tools: true, vision: true, contextWindow: 400000 } },
  { value: 'gpt-5.5', label: 'GPT-5.5', provider: 'openai', description: 'Previous flagship', capabilities: { thinking: true, tools: true, vision: true, contextWindow: 400000 } },
  { value: 'gpt-5.4', label: 'GPT-5.4', provider: 'openai', description: 'Legacy', capabilities: { thinking: true, tools: true, vision: true, contextWindow: 400000 } },
  ...
  { value: 'gpt-5.2', label: 'GPT-5.2', provider: 'openai', description: 'Legacy', apiKeyOnly: true, capabilities: { thinking: true, tools: true, vision: true, contextWindow: 400000 } },
...
export const AUTO_PHASE_THINKING: PhaseThinkingConfig = {
  spec: 'medium',
  planning: 'medium',
  coding: 'low',
  qa: 'low'
...
export const COMPLEX_PHASE_THINKING: PhaseThinkingConfig = {
  spec: 'high',
  planning: 'high',
  coding: 'high',
  qa: 'high'
...
export const BALANCED_PHASE_THINKING: PhaseThinkingConfig = {
  spec: 'medium',
  planning: 'medium',
  coding: 'medium',
  qa: 'medium'
...
export const QUICK_PHASE_THINKING: PhaseThinkingConfig = {
  spec: 'low',
  planning: 'low',
  coding: 'low',
  qa: 'low'
...
export const DEFAULT_PHASE_THINKING: PhaseThinkingConfig = BALANCED_PHASE_THINKING;
...
const OPENAI_COMPATIBLE_FEATURE_MODELS: FeatureModelConfig = {
  insights: 'gpt-5.6-sol',
  ideation: 'gpt-5.6-sol',
  roadmap: 'gpt-5.6-sol',
  githubIssues: 'gpt-5.6-sol',
  githubPrs: 'gpt-5.6-sol',
  utility: 'gpt-5.6-sol',
  naming: 'gpt-5.6-sol'
};

const OPENAI_COMPATIBLE_FEATURE_THINKING: FeatureThinkingConfig = {
  insights: 'medium',
  ideation: 'high',
  roadmap: 'high',
  githubIssues: 'medium',
...
const OLLAMA_FEATURE_THINKING: FeatureThinkingConfig = {
  insights: 'low',
  ideation: 'low',
  roadmap: 'low',
  githubIssues: 'low',
 ...
 */
export const PROVIDER_PRESET_DEFINITIONS: Partial<Record<ModelCatalogProvider, Record<string, ProviderPresetConfig>>> = {
  anthropic: {
  ...
  openai: {
    auto:     { primaryModel: 'gpt-5.6-sol', primaryThinking: 'medium', phaseModels: { spec: 'gpt-5.6-sol', planning: 'gpt-5.6-sol', coding: 'gpt-5.6-sol', qa: 'gpt-5.6-sol' }, phaseThinking: { spec: 'medium', planning: 'medium', coding: 'low', qa: 'low' } },
    complex:  { primaryModel: 'gpt-5.6-sol', primaryThinking: 'xhigh',  phaseModels: { spec: 'gpt-5.6-sol', planning: 'gpt-5.6-sol', coding: 'gpt-5.6-sol', qa: 'gpt-5.6-sol' }, phaseThinking: { spec: 'xhigh', planning: 'xhigh', coding: 'xhigh', qa: 'xhigh' } },
 ...
 */
export function getProviderPreset(provider: ModelCatalogProvider, presetId: string): ProviderPresetConfig | null {
 ...
 */
export function getProviderPresetOrFallback(provider: ModelCatalogProvider, presetId: string): ProviderPresetConfig {
  // Try exact match
...
export function getProviderDefaultFeatureModels(provider?: BuiltinProvider | string): FeatureModelConfig {
  ...
  if (provider === 'ollama') {
    return OLLAMA_FEATURE_MODELS;
  }

  return DEFAULT_FEATURE_MODELS;
}

export function getProviderDefaultFeatureThinking(provider?: BuiltinProvider | string): FeatureThinkingConfig {
  if (provider === 'deepseek') {
  ...
  if (provider === 'ollama') {
    return OLLAMA_FEATURE_THINKING;
  }
...
export const LEGACY_THINKING_MAP: Record<string, string> = {
  max: 'xhigh',
  ultra: 'xhigh',
  ultrathink: 'high',
  none: 'low',
...
export const DEFAULT_MODEL_EQUIVALENCES: Record<string, Partial<Record<BuiltinProvider, ProviderModelSpec>>> = {
  ...
  'gpt-5.4': {
    openai: { modelId: 'gpt-5.4', reasoning: { type: 'reasoning_effort', level: 'high' } },
    anthropic: { modelId: 'claude-opus-4-6', reasoning: { type: 'adaptive_effort', level: 'high' } },
    google: { modelId: 'gemini-2.5-pro', reasoning: { type: 'thinking_toggle', level: 'high' } },
  },
  'gpt-5.3-codex': {
 ...
 */
export function inferProviderFromModelValue(modelValue: string): ModelCatalogProvider | undefined {
 ...
 */
export function detectProviderFromModelId(modelValue: string): ModelCatalogProvider {
...
export function resolveModelEquivalent(
  ...
  if (targetProvider === 'openai-compatible') {
    const openAIEquivalent = DEFAULT_MODEL_EQUIVALENCES[modelValue]?.openai;
    ...
    for (const spec of Object.values(providerMap)) {
      if (spec?.modelId === modelValue) {
        const targetSpec = providerMap[targetProvider]
          ?? (targetProvider === 'openai-compatible' ? providerMap.openai : undefined);
