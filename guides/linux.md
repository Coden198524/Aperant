...
```bash
flatpak install --user apps/desktop/dist/Autocode-*.flatpak
```

### Running from Flatpak

```bash
flatpak run com.autoclaude.Autocode
```
...
```bash
# Make executable
chmod +x Autocode-*-linux-x86_64.AppImage

# Run
./Autocode-*-linux-x86_64.AppImage
```
...
```bash
sudo dpkg -i Autocode-*-linux-amd64.deb
```
...
ldd ./Autocode-*-linux-x86_64.AppImage

# Try running with debug output
./Autocode-*-linux-x86_64.AppImage --verbose
```
